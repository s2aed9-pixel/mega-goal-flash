...
/*geo
this piece of code demonstrates show all complete && match!
...
function initPhrases() {
	...
	//
	if (numPhrases >= theMovie.unlockCheckAllNo) {
		theMovie.showAll = 1;
	...
	if (this.clicked == 0) {
		...
		for (i=0; i<theMovie.phrasesToMatch; i++) {
			...
			if (theMovie["matchCheck"+i+"_mc"].setMeOn==1){
			    ...
			    if ((theMovie["matchCheck"+i+"_mc"].keepMyCurrentFrame==4) || (theMovie["matchCheck"+i+"_mc"].keepMyCurrentFrame==5)){
			// AUDIOScript Justification
			theMovie["audioJustify_"+i]._visible = true;
			theMovie["audioJustify_"+i].gotoAndStop(1);
			theMovie["audioJustify_"+i].pressed = false;
...
function checking() {  
	...
	if (((hitclip!="none") && (found == 1) && (this.myType == 1)) || ((hitclip!="none") && (this.myText == hitclip.myText) && (this.myType == 2))) {
			this._x = hitclip._x;
			this._y = hitclip._y;
			this.swapDepths(this.myDepth);
			correct += 1;
		...
		if (((hitclip!="none") && (hitclip.put==0) && (this.id!=hitclip.id) && (this.myType == 1)) || ((hitclip!="none") && (this.myText!=hitclip.myText) && (this.myType == 2))) {
			//this._x = this.startX;
			//this._y = this.startY;
			//theMovie["myLine"+this.id+"_mc"].removeMovieClip();
			hitclip.put = 1;
