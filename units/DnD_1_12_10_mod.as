...
4. When last item was placed back to items checkAll button wasn't turn to grey Line 337
*/
...
var hitI = this.myPlaceIDi;
var hitJ = this.myPlaceIDj;
			...
			for (var m = 0; m<myArrayItems.length; m++) {
				...
				if (theMovie[dragName].myNo == myArrayItems[m]) {
					...
					if (theMovie.itemsWillSubsAccToPlace) {
						...
						theMovie[dragName].my_txt.text = theMovie.items_array[theMovie[dragName].myId];
					}			
				}
			}
...
function solutionBut_mc_onRelease() {
	if (this._currentframe == 1) {
		...
		//MOD:10/11/11
		if (theMovie._parent.checkAll_mc.rightframe != 2){
			theMovie._parent.checkAll_mc.enabled = true;
		}
		...
		for (var i = 0; i < theMovie.items_array.length; i++) {
			for (var j = 0; j < theMovie.maxPlaces; j++) {
				...
				if (theMovie["ddCheck" + i + j + "_mc"].goframe != 3){
					//MOD:10/11/11
					theMovie["ddCheck" + i + j + "_mc"].enabled=true;
				}
...
function itemX_mc_onRelease() {
	...
	if ((hitTheRoad <> -1) && (theMovie[dropName].lockPlace == 0)) {
		...
		for (var i = 0; i < theMovie.maxCategories; i++) {
			for (var j = 0; j < theMovie.maxPlaces; j++) {
				//MOD:10/11/11 
				if (theMovie["ddCheck" + i + "" + j + "_mc"]._currentframe <> 3 && theMovie["ddCheck" + i + "" + j + "_mc"]!= undefined) {
					checkDownButton = 1;
				}
			}
		}
...
function initDragNDropCat() {
	...
	for (var i = 0; i < theMovie.maxCategories; i++) {
		for (var j = 0; j < theMovie.maxPlaces; j++) {
			...
			theMovie["ddCheck" + i + j + "_mc"].myPlaceIDi = i;
			theMovie["ddCheck" + i + j + "_mc"].myPlaceIDj = j;
